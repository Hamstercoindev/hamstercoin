...
#if defined(NDEBUG)
# error "Hamstercoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0a70e1319b8036477df1b83810144722e7634e655ff26e544929a4143f739c9a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Hamstercoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Hamstercoin Signed Message:\n";
...
{
    // Hamstercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
if(nHeight == 1)   
    ...
    if (nHeight != 0)
	{
        for (int Loop = 0; Loop < nHeight /5; Loop++)
		{
            	nSubsidy = (nSubsidy/10)*9;
        }
	}
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 15 * 60 * 60; // Hamstercoin: 2 hours
static const int64 nTargetSpacing = 10 * 60; // Hamstercoin: 20 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Hamstercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Hamstercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0a70e1319b8036477df1b83810144722e7634e655ff26e544929a4143f739c9a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "22.05.17: This is Hamstercoin! Hamsterin schäme dich! (DE)";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042184710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        ...
        assert(block.hashMerkleRoot == uint256("0x2fcf321db179110b36a2c3c0a1461697cfcb0345fa1093ad725fbb55638baad1"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Hamstercoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HamstercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HamstercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HamstercoinMiner(CWallet *pwallet)
{
    printf("HamstercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hamstercoin-miner");
    ...
    try { loop {
        ...
        printf("Running HamstercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HamstercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HamstercoinMiner, pwallet));
}
